...
for i in range(100, 1100, 100):
    ...
    py_int[0].append(
        min(
            timeit.Timer("a = a + b", "from __main__ import a, b").repeat(
    ...
    py_int[1].append(
        min(
            timeit.Timer("a += b", "from __main__ import a, b").repeat(
                repeat=3, number=1000
            )
        )
    )
...
def plot():
    ...
    ax[1].set_title(
        "Addition Python list objects\n" "(where a and x are lists w. length N)"
    )
    ax[2].set_title(
        "NumPy: In-place operator for element-wise"
