...
for sample in samples:
    ...
    X, y = make_classification(
        n_samples=sample, n_features=int(0.01 * sample), n_classes=2
    )

    # Set training and validation sets
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=int(0.1 * sample)
    )
...
tune_search = TuneGridSearchCV(
    SGDClassifier(), parameters, early_stopping="MedianStoppingRule", max_iters=10
)
