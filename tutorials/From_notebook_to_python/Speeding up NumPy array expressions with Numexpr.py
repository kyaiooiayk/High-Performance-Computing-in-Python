...
for n in orders_n:
    for f in funcs:
        ...
        timings_np[f].append(
            min(
                timeit.Timer(
                    "numpy_%s(A, B)" % f, "from __main__ import A, B, numpy_%s" % f
        ...
        timings_ne[f].append(
            min(
                timeit.Timer(
                    "numexpr_%s(A, B)" % f, "from __main__ import A, B, numexpr_%s" % f
                ).repeat(repeat=3, number=1)
            )
        )
...
def summary_table(funcs):
    fit_table = prettytable.PrettyTable(
        ["n=%s" % orders_n[-1], "Numexpr function", "rel. performance gain via Numexpr"]
    )
    fit_table.align["Numexpr function"] = "l"
    for f in funcs:
        fit_table.add_row(
            ["", f, "{:.2f}x".format(timings_np[f][-1] / timings_ne[f][-1])]
        )
    print(fit_table)
...
def plot_figures(funcs):
    ...
    for i, f in enumerate(funcs):
        ...
        plt.plot(
            [i**2 for i in orders_n],
            [i * 100 for i in timings_np[f]],
            alpha=0.5,
            label="NumPy %s" % f,
            marker="o",
        )
